...
import com.lnbei.system.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl implements ISysMenuService
{
    ...
    public List<SysMenu> selectMenusByUser(SysUser user)
    {
        ...
        if (user.isAdmin())
        {
            menus = menuMapper.selectMenuNormalAll("0");
        }
        else
        {
     ...
     */
    @Override
    public List<SysMenu> selectMenusByMemberUser(Object memberUser)
    {
    	SysUser sysUser = new SysUser();
    	MemberUser users = new MemberUser();
    	if (memberUser instanceof SysUser ) {
			sysUser = (SysUser) memberUser; 
		}else if (memberUser instanceof MemberUser )  {
			users = (MemberUser) memberUser;
		}else {
			return null;
		}
        List<SysMenu> menus = new LinkedList<SysMenu>();
        ...
        {
            menus = menuMapper.selectMenuNormalAll("1");
        }
        else
        {
            menus = menuMapper.selectMemberMenusByMemberId(users.getMid());
        }
        return getChildPerms(menus, "0");
    }
    @Override
    public List<SysMenu> selectMenusByShopMemberUser(Object shopMemberUser){
    	SysUser mUser = new SysUser();
    	MemberUser memberUser = new MemberUser();
    	if (shopMemberUser instanceof SysUser ) {
			mUser = (SysUser) shopMemberUser; 
		}else {
			memberUser = (MemberUser) shopMemberUser;
		}
        ...
        {
            menus = menuMapper.selectMenuNormalAll("2");
        }
        else
        {
            menus = menuMapper.selectShopMenusByMemberId(memberUser.getMid());
        }
        return getChildPerms(menus, "0");
     ...
     */
    @Override
    public List<Ztree> roleMenuTreeData(String menuAttr, SysRole role)
    {
    	String roleId = role.getRoleId();
        List<Ztree> ztrees = new ArrayList<Ztree>();
        List<SysMenu> menuList = menuMapper.selectMenuAllByMenuAttr(menuAttr);
     ...
     */
    @Override
    public List<String> selectMemberMenuTree(String ranksId, String menuAttr){
    	return menuMapper.selectMemberMenuTree(ranksId, menuAttr);
    }
    
    
	@Override
	public List<Ztree> memberRanksMenuTreeData(String menuAttr, MemberUserRanks memberUserRanks) {
		
    	String rankId = memberUserRanks.getMid();
        List<Ztree> ztrees = new ArrayList<Ztree>();
        List<SysMenu> menuList = menuMapper.selectMenuAllByMenuAttr(menuAttr);
        if (StringUtils.isNotNull(rankId))
        {
            List<String> roleMenuList = menuMapper.selectMemberMenuTree(rankId, menuAttr);
            ztrees = initZtree(menuList, roleMenuList, true);
        }
        else
        {
    ...
    public String checkMenuNameUnique(SysMenu menu)
    {
        ...
        SysMenu info = menuMapper.checkMenuNameUnique(menu.getMenuName(), menu.getParentId().toString());
     ...
     */
    @Override
    public Set<String> selectPermsByMemberIdAndIdentity(String userId, String identity)
    {
        List<String> perms = menuMapper.selectPermsByMemberIdAndIdentity(userId,identity);
        Set<String> permsSet = new HashSet<>();
        for (String perm : perms)
        {
            if (StringUtils.isNotEmpty(perm))
    ...
    public List<SysMenu> selectMenuAllByMenuAttr(String menuAttr){
    	return menuMapper.selectMenuAllByMenuAttr(menuAttr);
    }
