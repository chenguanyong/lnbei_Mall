		...
		this.getUrl = function(url, hash, manual_trigger, method, data, updateView, extraParams) {
			...
			.done(function(result) {
				...
				if(updateView) {
					...
					var patt = new RegExp("<title>","g");
						
					var result_array = patt.exec(result);
					var patt = new RegExp("</title>","g");						
					var result_arrays = patt.exec(result);
					var titles = "未知";
					if(result_array != null && result_arrays != null){
					
						titles = result.substr(result_array.index+7,result_arrays.index-9);
					}
					titles = String(titles)
						.replace(/<(title|link)([\s\>])/gi,'')
						.replace(/<\/[a-z]{0,8}/gi,'')	;	
					...
					/*
					if($contentArea[0].className == "page-content-area"){
						$('[data-name="item"]').removeClass('active');
						$('#tab_content').children().removeClass('active');
						var d = new Date();																
						var seconds = d.getMilliseconds();
						$('[data-name=next]').before("<li data-name='item' class='active'><a data-toggle='tab' href='#menu"+seconds+"'>"+titles+" <i data-name='closetab' onclick='closetab(this)' class='fa fa-times-circle'></i></a></li>");
						$('<div role="tabpanel" class="tab-pane active" id="menu'+seconds+'">'+result+'</div>').appendTo("#tab_content");	
